      ...
      break;
       ...
       useEffect(()=>{
        ...
        const interval=setInterval(()=>{
          setSnake(prev=>{
             if(gameOver || !running) return prev ;
           const newHead = [
           ...
           prev[0][1] + directionRef.current[1]
            ...
            //collision
            if(newHead[0]<0 || newHead[0]>=Board|| newHead[1]<0 || newHead[1]>=Board
       ...
       },[ gameOver,running,player.id,dispatch,Direction,score,Food]);
     ...
     return(
       ...
       <Link to='/'>
        ...
        <div className='absolute bottom-6 -right-[6px]'>
          <img src="https://i.pinimg.com/originals/0e/b1/46/0eb146bd08bd17aeee67aeb300262225.gif" className='scale-x-[-1] z-10'/>
        </div>
        <div className='text-[3.5rem] font-bold text-pink-700 mt-4 '><u> Snake Game</u></div>
        <div className='flex justify-center items-center p-5'>
        {!gameOver && (
          <div className=' -ml-[12rem] mr-4 -mt-[5rem] space-y-8'>
      <div className='text-[25px] font-bold text-yellow-600 mx-3 mb-6 px-3'> PlayerName : {player.name} <br /> is playing the Game now ...</div>
       <div className='text-3xl font-bold text-yellow-500 m-3 '> Your Score : {score} </div>
       </div>
        ) }
       ...
       <div className=' relative w-[544px]  h-[544px]  bg-yellow-500 border-[10px] border-dashed border-pink-700 border-spacing-96 shadow-2xl shadow-black'>
          {!gameOver && (
            snake.map(([x,y],index)=>{ 
                    ...
                    return(    
              <div key={index}
                className='absolute border-2 border-red-800 shadow-md border-double shadow-black'
                style={{
          ...
          style={{
         ...
         
           {
            ...
            <Link to='/'>
             <button onClick={ClickMusic} className='rounded-xl px-10 py-4 text-3xl font-bold mb-1
              text-green-600 bg-red-800  border-[7px] border-double
               border-green-800  shadow-2xl shadow-black
                hover:bg-red-900 tansition-all duration-250'>Restart</button> 
             </Link>
            </div>
          )
           }
        ...
        {!gameOver && (
            <div className='absolute top-[1.5rem] right-44'>
